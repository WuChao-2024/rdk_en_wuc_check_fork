...
Operation modifiers:
  ...
  -n, --seek                 archive can be seeked
      ...
      --sparse-version=MAJOR[.MINOR]
                             set version of the sparse format to be used (implied by --sparse)-S, --sparse               Efficiently handle sparse files
...
gnu GNU tar 1.13.x formatoldgnu                   GNU format as per tar <= 1.12
...
-h, --dereference
                             follow symlinks; archive and output files they point to--hard-dereference
                             Track hard links; archive and output files they point to
...
nil, existing    make numbered backups if numbered backups already exist, otherwise make simple backups
never, simple    always make simple backups--quoting-style option has the following valid parameters:
